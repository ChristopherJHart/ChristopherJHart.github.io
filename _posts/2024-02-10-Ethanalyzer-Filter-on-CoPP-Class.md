...
> **Note**: If you get an error message similar to `tshark: Neither "cisco.blob.sup_qnum" nor "35" are field or protocol names.`, you most likely forgot to include the `decode-internal` keyword in the command. Otherwise, you may be running an older NX-OS software release - you'll need to upgrade the NX-OS software of your switch to NX-OS 10.1(1) or later to filter on traffic from a specific CoPP class in Ethanalyzer.
